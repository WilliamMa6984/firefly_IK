...
def constrain_angles(q):
    constraints = [pi, pi*160/180, pi*160/180, pi, pi*160/180, pi, pi]
...
def f_kine(angles):
    ...
    T6 = DH2tform(pi/2+th5, 0, 0, 0)
    T7 = DH2tform(th5_5, l4_5, 0, -pi/2)
    T8 = DH2tform(th6, l5, 0, 0)

    Tf_out = T1 @ T2 @ T3 @ T4 @ T5 @ T6 @ T7 @ T8
    # ===================== END USER TO MODIFY =====================
...
def graph_task(arg):
    ...
    target_Tf = f_kine(np.array([pi/2, pi/2, pi/2, pi/2, pi/2, pi/2, pi/2]))
